...
class StudyClubController extends Controller
{
    public function index()
    {
        ...
        return view('studyclub.index', compact('editions'));
    }

    public function edition($number)
    {
        $editions = $this->getEditions();
        $edition = collect($editions)->firstWhere('number', $number);

        if (!$edition) { abort(404); }

        return view('studyclub.edition', compact('edition'));
    }

    public function show($editionNumber, $itemId)
    {
        $editions = $this->getEditions();
        $edition = collect($editions)->firstWhere('number', $editionNumber);
        
        if (!$edition) { abort(404); }

        $item = collect($edition['items'])->firstWhere('id', $itemId);
        
        if (!$item) { abort(404); }

        // Artigos relacionados (outros da mesma edição ou edições anteriores)
        $relatedArticles = collect($edition['items'])->where('id', '!=', $itemId)->take(3);
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'likes' => 198,
                        'comments' => 21,
                        'external_url' => 'https://dentalgo.com.br/facelift25/artigo/1147/Clinical-2026-v25n01/16135/Ortho-Stable:-evolu%C3%A7%C3%A3o-conceitual-da-ancoragem-esquel%C3%A9tica-em-Ortodontia',
                        'icon' => 'bi-gear'
                    ]
                ]
            ],
