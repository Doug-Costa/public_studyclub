...
@php
  ...
  $showPromoBanner = ($currentDate >= '2026-03-25' && $currentDate <= '2026-03-29');
@endphp

@if($showPromoBanner)
<div id="promoBanner" style="
  background: linear-gradient(90deg, #d21d5b, #ff6b35, #d21d5b, #ff6b35);
  ...
  gap: 10px;
  box-shadow: 0 2px 8px rgba(210, 29, 91, 0.3);
  margin-top: 70px; /* Ajuste para não ficar por baixo do header fixo (64px base) */
">
  <i class="fa-solid fa-sparkles" style="font-size: 1.1rem;"></i>
  <span>
    🎉 <strong>Semana de acesso livre!</strong> De 25 a 29 de março, revistas, vídeos e o <a href="{{ route('gointelligence.index') }}" style="background: rgba(255,255,255,0.25); padding: 2px 10px; border-radius: 20px; font-weight: 800; letter-spacing: 0.5px; color: #fff; text-decoration: none;">✨ GoIntelligence</a> estão liberados gratuitamente. Aproveite!
  </span>
  <i class="fa-solid fa-sparkles" style="font-size: 1.1rem;"></i>
  <button onclick="document.getElementById('promoBanner').style.display='none'; localStorage.setItem('promoBannerClosed','1');"
    style="
      position: absolute;
      right: 15px;
      top: 50%;
...
<style>
  ...
  @media (max-width: 768px) {
    ...
    #promoBanner .fa-sparkles { display: none; }
  }
</style>
<script>
    ...
    $(document).ready(function() {
      ...
      if (modalVideo) {
        modalVideo.addEventListener('show.bs.modal', function (event) {
          ...
          const videoSrc = button ? button.getAttribute('data-src') : null;
          if (videoSrc) {
            $("#video").attr('src', "{{ route('videoplay') }}/" + videoSrc + "?v=" + new Date().getTime());
          }
        });
        modalVideo.addEventListener('hide.bs.modal', function () {
          $("#video").attr('src', "{{ route('loadingvideo') }}?v=" + new Date().getTime()); 
        });
      }
    });
</script>
...
@if (request()->getHost() === 'app.dentalgo.com.br')
    <style>
        .topbar {
...
@endif
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg modal-white">
            <div class="modal-content">
                <div class="modal-header-custom">
                    <button type="button" class="btn-close-custom" data-bs-dismiss="modal" aria-label="Close" style="position: fixed; margin-right: -5%;">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                <div class="modal-body p-0" style="margin-bottom: -6px !important;">
                    <iframe class="embed-responsive-item" src="{{ route('faceloadingvideo') }}?v={{ time() }}" id="video"
    ...
    <style>
        ...
        #modalVideo .modal-white .modal-content {
        ...
        #modalVideo .modal-header-custom {
            position: absolute;
            top: -50px;
            right: 0;
            padding: 10px;
        ...
        #modalVideo .btn-close-custom:hover {
